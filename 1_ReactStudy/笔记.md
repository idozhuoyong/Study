...
## 53. React样式私有化处理「styled-component」
...
redux/react-redux也是实现组件之间通信的技术「插件」，不管任何类型的组件，都可以基于这种方案，实现组件通信

公共状态管理方案

1. 在创建的 store 容器中，存储两部分内容 cont store = createStore([reducer])
   + 公共状态：各组件需要共享/通信的信息
...
2. 在组件内部，获取公用状态信息，然后渲染。store.getState() -> { supNum: xx, oppNum: xx }

3. 把让组件可以更新的方法，放在公共容器的事件池中！！后期公共状态改了，事件池中的方法会按照顺序，依次执行，也就是让对应的组件也更新，组件只要更新，就可以从 store 容器中获取最新的状态渲染。store.subscribe(函数)
...
1. 创建store，规划出reducer「当中的业务处理逻辑可以后续不断完善，但是最开始reducer的这个架子需要先搭建起来」
...
3. 组件中基于 store，完成公共状态的获取和任务的派发

   1. 使用到公共状态的组件，必须向store的事件池中加入让组件更新的方法，只有这样，才可以保证，公共状态改变，可以让组件更新，才可以获取最新的状态进行绑定
...
## 58. redux工程化-reducer的拆分和合并

在真正的项目中，我们一定会把状态和reucer的管理，按照模块化进行划分
...
```js
/* personal板块要派发的行为对象的管理 */
import * as TYPES from "../action-types";
const personalAction = {
    // "xx": TYPES.PERSONAL_INFO
};
export default personalAction;
```

合并action，actions/index.js
